...
void drawRectDMA(int row, int col, int width, int height, volatile u16 color) {
  volatile unsigned short color1;
  color1 = color;
  for (int count = 0; count < height; count++) {
    DMA[3].src = &color1;
    DMA[3].dst = &videoBuffer[OFFSET(row + count, col, WIDTH)];
...
void drawFullScreenImageDMA(const u16 *image) {
  for (int count = 0; count < HEIGHT; count++) {
    DMA[3].src = image + (WIDTH * count);
    DMA[3].dst = &videoBuffer[OFFSET(count, 0, WIDTH)];
...
void drawImageDMA(int row, int col, int width, int height, const u16 *image) {
  for (int count = 0; count < height; count++) {
    DMA[3].src = image + (width * count);
    DMA[3].dst = &videoBuffer[OFFSET(count + row, col, WIDTH)];
    DMA[3].cnt = width | DMA_ON | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT;
  }
}
